% ---------------------------------------------------------------
% This code implements a network of 2000 Lu subsystems 
...
X_bound = [-20 20;-20 20; -20 20]; % state space 

%======================= System parameters ================================

T = 11;   % Number of collected data

tau = 0.01;  % Sampleing time

number_subsystems = 2000;  % Number of subsystems

interconnected_subs = 1; % As subsystems are interconnected through star topology, one subsystem affects others.

n_states = 3; % Number of states

%=============== Subsystem (A_i, B_i, D_i) matrices (Lu) ==================

a = 36; %Lu system parameter 
c = 28;  %Lu system parameter 
b = 20;  %Lu system parameter 
...
D_coeff = 0.001; % Interconnection coefficient
...
initial = 0.5 * [2; 2; 2]; % Initial state vector [x_1; x_2; x_3]
...
X1T = statesdot.'+ (-0.12 + 0.24 * rand(n_states,T));

%Dictionary containing all combinations of system monomials up to degree 2 trajectory
N0T = N0T.';


clear tau initial states  statesdot x0 u t X i odeSystem tspan
...
bar_varkappa = n_states * 0.0144  % In equation (11)
...
Lambda_x = sosgetsol(prog,Lambda_x);
...
[prog,Mon_phi]  = sospolyvar(prog,Mon_phi);  % Constant phi
...
prog = sosineq(prog, Barrier - Lambda_u_2.'*b_uu  - beta); % Condition 20c for third unsafe region
...
for i = 1 : number_subsystems
    ...
    if i == 1
        ...
        compose_mat(i, 1) = (1/(Phi* PI))*(norm(Matrix_D))^2; 
...
rho1 = (1/(PI))*(norm(Matrix_D))^2 %Intraction gain
