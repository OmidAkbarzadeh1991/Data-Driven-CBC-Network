% ---------------------------------------------------------------
% This code implements a network of 1023 Duffing oscillator subsystems 
...
number_subsystems = 1023; % Number of subsystems
...
%========= Subsystem matrices (A_i, B_i, D_i) (Duffing oscillator) ========
...
for i = 1:T
    ...
    M  = [x0(1), x0(2), x0(1)^3]; 
     ...
     % Dictionary containing all combinations of system monomials up to degree 3.
    M1 = [x0(1), x0(2), x0(1) * x0(2), x0(1)^2, x0(2)^2, x0(1)^2 * x0(2), x0(1) * x0(2)^2, x0(1)^3, x0(2)^3]; % Choice
...
Upsilon_x  = [1 0;0 1; x_2 0; x_1 0; 0 x_2; 0 x_1^2; x_2^2 0 ; x_1^2 0;0 x_2^2];
...
Barrier_certificate_to_show = x.' * P * x
...
g0_2 = (x2 - X0_bound(2, 1))*(X0_bound(2, 2) - x2);
...
prog = sosineq(prog, Barrier - Lambda_u.'*b_u - beta);    % Condition 20c for first unsafe region

prog = sosineq(prog, Barrier - Lambda2_u.'*b_uu - beta);    %Condition 20c for second unsafe region
...
for kk = 1:length(Lambda_u)
    prog = sosineq(prog,Lambda_u(kk));
end

for zz = 1:length(Lambda2_u)
    prog = sosineq(prog,Lambda2_u(zz));
end
...
SOLV1 = sosgetsol(prog, Barrier - Lambda_u.'*b_u - beta);
SOLV2 = sosgetsol(prog, Barrier - Lambda2_u.'*b_uu - beta);
...
for j = 1 : number_subsystems
    compose_mat(j, j) = -epsilon;
    for i = 1 : number_subsystems
        if i == 2 * j
            if i + 1 > number_subsystems
                compose_mat(i, j) = (1/(Phi * PI))*(norm(Matrix_D))^2;
            else
                compose_mat(i : i + 1, j) = (1/(Phi  * PI))*(norm(Matrix_D))^2;
            end
        end
    end
end
