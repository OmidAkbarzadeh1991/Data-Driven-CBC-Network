% ---------------------------------------------------------------
% This code implements a network of 2000 spacecraft subsystems 
...
X_bound   = [-5 5;-5 5;-5 5];  % Space state

%======================= System parameters ================================

number_subsystems = 2000; % Number of subsystems

interconnected_subs = 1; % As subsystems are interconnected through line topology, one subsystem affects the other one.

T = 14;    % Number of collected data

tau = 0.05;   % Sampleing time

initial = -0.1 * [0.5; 0.5; 0.5]; % Initial state vector [x_1; x_2; x_3]

n_states = 3; % Number of states

%============= Subystem (A_i, B_i, D_i) matrices (Spacecraft) =============

j1 = 20; % Spacecraft parameter
j2 = 20; % Spacecraft parameter
j3 = 30; % Spacecraft parameter

A = blkdiag((j2 - j3)/j1, (j3 - j1)/j2, (j1 - j2)/j3);
A = [zeros(3, 3) A];
...
for i = 1:T
  ...
  % Original vector of system monomials: 
    M = [x0(1), x0(2), x0(3), x0(2)*x0(3), x0(3)*x0(1), x0(1)*x0(2)];


   % Dictionary containing all combinations of system monomials up to degree 2.
    M1 = [x0(1), x0(2), x0(3), x0(2)*x0(3), x0(3)*x0(1), x0(1)*x0(2), x0(1)^2, x0(2)^2, x0(3)^2]; 

    % Random input
    u(:, i) = 5 * (-100 + 200 * rand(3, 1)); 
    ...
    for k = 1 : interconnected_subs
      ...
      w = 0.08 * [-10 + 20*rand -10 + 20*rand -20 + 20*rand];
...
X1T = statesdot.'+ (-0.5 + rand(n_states,T));

%Dictionary containing all combinations of system monomials up to degree 2 trajectory
N0T = N0T.';

clear tau initial initial1 states states_bar statesdot statesdot_bar x0  x01 u t X i odeSystem odeSystem1 tspan
...
Upsilon_x = [1 0 0;0 1 0;0 0 1;0 0 x_2;x_3 0 0; x_2 0 0;x_1 0 0;0 x_2 0; 0 0 x_3];
...
bar_varkappa = n_states * 0.25 %In equation (11)
...
Condition_3_creation = [G  H_x.'; H_x  mu * eye(T)] - lambda_x.' * b_x * eye(T + n_states);
...
gamma_network = number_subsystems * gamma

beta_network = number_subsystems *  beta
...
compose_mat = zeros(number_subsystems, number_subsystems);
...
rho = (1/(PI))*(norm(Matrix_D))^2  %Intraction gain
